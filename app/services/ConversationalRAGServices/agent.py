...
async def chat_with_agent(user_query: str, session_id: str)-> str:
    ...
    message = [("system", "You are a helpful assistant. Answer questions using the provided context and conversation history. Keep responses clear, accurate, and relevant."),
            ("user",prompt)]
    
    response_obj = llm.invoke(message)
    response_text = response_obj.content
    save_to_history(session_id, "user", user_query)
    save_to_history(session_id, "assistant", response_text)
